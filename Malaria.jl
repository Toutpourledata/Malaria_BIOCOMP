...
Malaria_equations = [
    ...
    D(Vs) ~ Z * (1 + 1*cos( 2* pi*t / 365 ))     - λ*Hp*Vs - Vs*ψv,
    D(Vp) ~      λ*Hp*Vs                         - Vp*ψv
...
param = [
        ...
        ψh => 0.0005,  #Taux de sortie des humains
        ψv => 0.01,  #Taux de sortie des vecteurs
        μh => 0.005,  #Taux de mortalité dû au parasite
        λ => 0.00001,  #Taux de transmission
        β => 0.8,   #Taux de guérison
        δ => 0.5,  #Taux de perte d'immunité
        θ => 0.1]   #Taux de vaccination

# conditions initiales
u0 = [
    Hs => 5000,   # humains susceptibles (black)
    Hp => 100,   # humains parasités (red)
    Hr => 30,    # humains rétablis (green)
...
# graphique
fig = Figure(; resolution=(1000, 1000), layout=(2,2))
...
ylims!(timecourse_human, (0., 7000.))

# vecteurs 
timecourse_vector = Axis(fig[1, 2]; xlabel="Temps", ylabel="Population vecteur")
...
# dynamique humain
phaseportrait_humain = Axis(fig[2,1]; xlabel="Humains susceptibles", ylabel="rétablis")

xlims!(phaseportrait_humain, (0., 5500.))
ylims!(phaseportrait_humain, (0., 2500.))
#zlims!(phaseportrait_humain, (0., 1000.))

lines!(phaseportrait_humain, sol[1, :], sol[3, :],  color=:black)

#dynamique vecteur
phaseportrait_vecteur = Axis(fig[2,2]; xlabel="Proies", ylabel="Prédateurs")

xlims!(phaseportrait_vecteur, (0., 70000.))
ylims!(phaseportrait_vecteur, (0., 50000.))

lines!(phaseportrait_vecteur, sol[4, :], sol[5, :], color=:black)
